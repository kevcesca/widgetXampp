class scriptsSears extends HTMLElement {
    ...
    connectedCallback() {
        ...
        do {
            setTimeout(() => {
                ...
                if (interaccionPreview == null) {
                    ...
                    console.log(interaccionPreview);
                    control= 1;
                }
            }, 1000);
        } while (control == 1);
        ...
        this.interactionId = this.getAttribute("interactionid") ? this.getAttribute("interactionid") : interaccionPreview;
        ...
        this.api.onDataEvent('onMessageEvent', (data) => {
            ...
            if (data && data.cuenta && data.tarjeta && data.nombre && data.motivo) {
                ...
                console.log(`Cuenta: ${cuenta}, Tarjeta: ${tarjeta}, Nombre: ${nombreCliente}, Motivo: ${motivo}`);
        ...
        if (this.workRequestId == !null) {
            ...
            _API.onDataEvent('onInteractionEvent', function (data) {
                if (data.state == "ALERTING" || data.state === "ACTIVE") {
                    ...
                    const url = `https://127.0.0.1/widgets/scripts-sears/index.html?cuenta=${encodeURIComponent(cuenta)}&tarjeta=${encodeURIComponent(tarjeta)}&motivo=${encodeURIComponent(motivo)}&telefono=${encodeURIComponent(telefono)}&queue=${encodeURIComponent(queue)}&nombreAgente=${encodeURIComponent(nombreAgente)}&nombreCliente=${encodeURIComponent(nombreCliente)}`;
