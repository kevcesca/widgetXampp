class scriptsSears extends HTMLElement {
    ...
    // Función para generar la URL y actualizar el iframe, sustituyendo valores faltantes por ""
    generateIframeUrl($container) {
        // parametros desde el messageEvent con lowinco
        const tarjeta = this.dataFromMessageEvent.cuenta || '';
        const cuenta = this.dataFromMessageEvent.tarjeta || '';
        const nombreCliente = this.dataFromMessageEvent.nombreCliente || '';
        const motivo = this.dataFromMessageEvent.motivo || '';
        // parametros desde el interactionEvent
        const queue = this.dataFromInteractionEvent.queue || '';
        const telefono = this.dataFromInteractionEvent.telefono || '';
        const nombreAgente = this.dataFromInteractionEvent.nombreAgente || '';

        // Generamos la URL con los valores, usando "" en los valores que no estén disponibles
        const url = `https://127.0.0.1/widgets/scripts-sears/index.html?cuenta=${encodeURIComponent(cuenta)}&tarjeta=${encodeURIComponent(tarjeta)}&motivo=${encodeURIComponent(motivo)}&telefono=${encodeURIComponent(telefono)}&queue=CEAT%20Sanborns&nombreCliente=${encodeURIComponent(nombreCliente)}&nombreAgente=${encodeURIComponent(nombreAgente)}`;
